...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("four_weeks_of_sleep_score_in_one_call",
			mcplib.WithDescription("The score trend is server-aggregated, so a month costs one request instead of twenty-eight per-night calls."),
			...
			mcplib.WithString("slug3", mcplib.Required(), mcplib.Description("Override the recipe's positional slug value.")),
		),
		handleFourWeeksOfSleepScoreInOneCall,
	)
}

// handleFourWeeksOfSleepScoreInOneCall runs the four_weeks_of_sleep_score_in_one_call recipe intent: The score trend is server-aggregated, so a month costs one request instead of twenty-eight per-night calls.
func handleFourWeeksOfSleepScoreInOneCall(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	var missingSlug3 bool
	args, missingSlug3 = appendRecipePositional(args, input["slug3"], true)
	if missingSlug3 {
		return mcplib.NewToolResultError("slug3 is required"), nil
	}
