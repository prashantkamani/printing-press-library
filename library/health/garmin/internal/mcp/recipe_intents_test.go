...
func TestRecipeIntentHandlerBuildsRecipeArgs(t *testing.T) {
	...
	req := mcplib.CallToolRequest{Params: mcplib.CallToolParams{Arguments: map[string]any{
		"slug":  "required-value",
		"slug2": "required-value",
		"slug3": "required-value",
	}}}
	result, err := handleFourWeeksOfSleepScoreInOneCall(context.Background(), req)
	if err != nil {
...
func TestRecipeIntentHandlerOverridesParams(t *testing.T) {
	...
	if !strings.Contains(got, "required-value") {
		t.Fatalf("positional override for slug2 missing from %q", got)
	}
	if !strings.Contains(got, "required-value") {
		t.Fatalf("positional override for slug3 missing from %q", got)
	}
}
