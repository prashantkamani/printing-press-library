...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("GARMIN_ACCESS_TOKEN"); v != "" {
		cfg.AccessToken = v
		cfg.markEnvOverride("AccessToken")
		cfg.AuthSource = "env:GARMIN_ACCESS_TOKEN"
		cfg.CredentialSource = "env:GARMIN_ACCESS_TOKEN"
	}
	if v := os.Getenv("GARMIN_TOKEN"); v != "" {
		cfg.GarminToken = v
		cfg.markEnvOverride("GarminToken")
		cfg.AuthSource = "env:GARMIN_TOKEN"
		cfg.CredentialSource = "env:GARMIN_TOKEN"
	}
	...
	if v := os.Getenv("GARMIN_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("GARMIN_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "garmin-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.AccessToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:GARMIN_ACCESS_TOKEN"
		}
		return "Bearer " + c.AccessToken
	}
	if c.GarminToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:GARMIN_TOKEN"
		}
		return "Bearer " + c.GarminToken
	}
	return ""
}
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.GarminToken != "" {
		parts = append(parts, "token="+c.GarminToken)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.GarminToken == "" {
		c.GarminToken = creds.GarminToken
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.GarminToken = ""
	delete(c.envOverrides, "GarminToken")
	c.updateFileConfigField("GarminToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	...
	if c.fileConfig != nil {
		...
		if c.envOverrides["GarminToken"] {
			out.GarminToken = c.fileConfig.GarminToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "GarminToken":
		c.fileConfig.GarminToken = c.GarminToken
	}
}
