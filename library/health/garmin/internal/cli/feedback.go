...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GARMIN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GARMIN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "garmin-pp-cli/feedback")
...
When ` + "`GARMIN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GARMIN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  garmin-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  garmin-pp-cli feedback --stdin < notes.txt
  garmin-pp-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  garmin-pp-cli feedback list
  garmin-pp-cli feedback list --limit 5
  garmin-pp-cli feedback list --json`,
