...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "auth",
		Short:       "Manage authentication for Garmin Connect Unofficial",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  garmin-pp-cli auth setup\n  garmin-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://sso.garmin.com/portal/sso/en-US/sign-in")
			fmt.Fprintln(w, "  Run `garmin-pp-cli auth login --email <your Garmin account email>`. It opens your browser at Garmin's sign-in page, catches the redirect back to a loopback port, exchanges the ticket for a token pair, and refuses to store a token whose account email does not match the one you named. Sign out of Garmin in the browser first if another household member has used it, and re-run `auth login` per account with that account's own GARMIN_HOME. There is no key to copy and paste: Garmin issues no personal API token, and the password is never seen by this CLI.")
			fmt.Fprintln(w, "  echo \"$TOKEN\" | garmin-pp-cli auth set-token")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Set one of:")
			fmt.Fprintln(w, "  export GARMIN_ACCESS_TOKEN=\"your-token-here\"")
			fmt.Fprintln(w, "  export GARMIN_TOKEN=\"your-token-here\"")
			if !launch {
				return nil
			}
			launchURL := "https://sso.garmin.com/portal/sso/en-US/sign-in"
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "")
				fmt.Fprintln(w, "Set one of:")
				fmt.Fprintf(w, "  echo \"$TOKEN\" | garmin-pp-cli auth set-token\n")
				fmt.Fprintln(w, "")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  echo \"$TOKEN\" | garmin-pp-cli auth set-token\n  garmin-pp-cli auth set-token < token-file",
		Args:    cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("GARMIN_ACCESS_TOKEN") != "" {
				envStillSet = "GARMIN_ACCESS_TOKEN"
			}
			if envStillSet == "" && os.Getenv("GARMIN_TOKEN") != "" {
				envStillSet = "GARMIN_TOKEN"
			}
