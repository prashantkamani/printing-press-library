...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/health/garmin/internal/learn/lookups"
...
Resource scoping:
  ...
  garmin-pp-cli sync --resources activities,heart_rate

  # Full resync (ignore previous checkpoint)
  garmin-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  garmin-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  garmin-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  garmin-pp-cli sync --latest-only`,
...
func determinePaginationDefaults(resource string) paginationDefaults {
	...
	case "activities":
		return paginationDefaults{
			// `start` is the wire name: the spec declares the parameter as
			// `offset` with `x-url-name: start`, and the press's pagination
			// detection reads param.Name, not the wire name, so a reprint
			// restores "offset" here. Garmin ignores an `offset` query key,
			// which pinned every walk to the newest page.
			cursorParam:    "start",
			cursorType:     "offset",
			nextCursorPath: "",
			limitParam:     "limit",
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "steps":
		return db.UpsertSteps(data)
	case "training":
		return db.UpsertTraining(data)
	case "wellness":
		return db.UpsertWellness(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		...
		"heart_rate-heart-rate-zones": "/biometric-service/heartRateZones",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "activity_service_activity", ParentTable: "activities", ParentIDParam: "activityId", PathTemplate: "/activity-service/activity/{activityId}", KeyField: "activityId", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.activity", PathParams: []dependentPathParamDef{
			{Param: "activityId", Field: "activity_id"},
		}},
		{Name: "activity_service_activity_hr_time_in_zones", ParentTable: "activities", ParentIDParam: "activityId", PathTemplate: "/activity-service/activity/{activityId}/hrTimeInZones", KeyField: "activityId", ReconcileMode: "none", GenericScopeJSONPath: "", PathParams: []dependentPathParamDef{
			{Param: "activityId", Field: "activity_id"},
		}},
		{Name: "activity_service_activity_splits", ParentTable: "activities", ParentIDParam: "activityId", PathTemplate: "/activity-service/activity/{activityId}/splits", KeyField: "activityId", ReconcileMode: "none", GenericScopeJSONPath: "", PathParams: []dependentPathParamDef{
			{Param: "activityId", Field: "activity_id"},
		}},
	}
}
...
func responsePathForResource(resource, path string) []string {
	...
	switch resource {
	case "heart_rate":
		return []string{"heartRateValues"}
	case "sleep":
		return []string{"individualStats"}
	}
	return nil
}
