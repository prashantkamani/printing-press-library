...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  garmin-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  garmin-pp-cli workflow archive --full

  # Archive without a wall-clock timeout
  garmin-pp-cli workflow archive --timeout 0`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  garmin-pp-cli workflow status

  # Show status as JSON
  garmin-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("garmin-pp-cli")
			}
