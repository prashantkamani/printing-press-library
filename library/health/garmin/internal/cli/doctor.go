...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  garmin-pp-cli doctor
  garmin-pp-cli doctor --json
  garmin-pp-cli doctor --fail-on warn
  garmin-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set one of: export GARMIN_ACCESS_TOKEN=\"your-token-here\" or export GARMIN_TOKEN=\"your-token-here\""
						report["auth_key_url"] = "https://sso.garmin.com/portal/sso/en-US/sign-in"
						report["auth_instructions"] = "Run `garmin-pp-cli auth login --email <your Garmin account email>`. It opens your browser at Garmin's sign-in page, catches the redirect back to a loopback port, exchanges the ticket for a token pair, and refuses to store a token whose account email does not match the one you named. Sign out of Garmin in the browser first if another household member has used it, and re-run `auth login` per account with that account's own GARMIN_HOME. There is no key to copy and paste: Garmin issues no personal API token, and the password is never seen by this CLI."
					} else {
						authConfigured = true
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "GARMIN_ACCESS_TOKEN")
				if os.Getenv("GARMIN_ACCESS_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "GARMIN_ACCESS_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("GARMIN_ACCESS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "GARMIN_ACCESS_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "GARMIN_ACCESS_TOKEN optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "GARMIN_TOKEN")
				if os.Getenv("GARMIN_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "GARMIN_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("GARMIN_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "GARMIN_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "GARMIN_TOKEN optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "garmin-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'garmin-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'garmin-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny:
		...
		report["hint"] = "No sync recorded; run 'garmin-pp-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'garmin-pp-cli sync' to refresh."
	}
	return report
}
