...
type rootFlags struct {
	...
	// invocation. Mirrors the GARMIN_NO_LEARN env var.
	noLearn                 bool
	selectFields            string
	configPath              string
	homePath                string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "garmin-pp-cli",
		Short: `Garmin CLI — Your whole Garmin history in a local database, not one 28-day page at a time.`,
		Long: `Garmin CLI — Your whole Garmin history in a local database, not one 28-day page at a time.
...
Highlights (not in the official API docs):
  ...
  • history   Fills a local SQLite archive from the oldest day forward, one bookmark per series, so an interrupted run continues where it stopped — 28-day windows where Garmin caps a request at 28 days, 364-day wi…
...
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.SetVersionTemplate("garmin-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select activityId,activityName,activityType)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'garmin-pp-cli profile list')")
	...
	rootCmd.AddCommand(newFitnessCmd(flags))
	rootCmd.AddCommand(newHeartRateCmd(flags))
	rootCmd.AddCommand(newSleepCmd(flags))
	rootCmd.AddCommand(newStepsCmd(flags))
	rootCmd.AddCommand(newTrainingCmd(flags))
	rootCmd.AddCommand(newWellnessCmd(flags))
	...
	if parent, _, err := rootCmd.Find(strings.Fields("auth")); err == nil {
		addNovelCommandIfAbsent(parent, newNovelAuthLoginCmd(flags))
	}
