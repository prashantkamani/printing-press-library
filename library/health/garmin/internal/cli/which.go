...
var whichIndex = []whichEntry{
	{Command: "insights sleep", Description: "Duration, score, stage split and resting heart rate for a window of nights, with the equally long window before it beside them.", Group: "Questions answered from the archive", WhyItMatters: "Pick this over per-night endpoint calls whenever the question spans more than a few nights: one local read replaces one request per night, and nights the watch did not measure are counted as missing rather than as zero."},
	{Command: "insights training", Description: "Archived activities grouped by type with their time and distance, time summed per heart-rate zone, weekly load buckets, and the VO2-max and readiness trends beside them.", Group: "Questions answered from the archive", WhyItMatters: "Pick this over the activity feed when the question is about volume, intensity or balance rather than one workout; it reports how many activities had zones archived so a strapless ride is not read as easy training."},
	{Command: "history", Description: "Fills a local SQLite archive of every Garmin daily series from the oldest day forward, keeping one bookmark per series so an interrupted run resumes instead of restarting.", Group: "Local state that compounds", WhyItMatters: "Run this before any question that reaches past the last 28 days: `--since all` or `--since YYYY-MM-DD` is the only depth control it offers, `--status` reports what each series still owes, and every insights command reads the archive rather than the API."},
	{Command: "auth login", Description: "Refuses to store a token whose account is not the address you named, so a shared browser cannot sign the wrong household member in.", Group: "Auth you can trust with a household", WhyItMatters: "Run this once per Garmin account per home before anything else; afterwards every command works unattended from the refresh token."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  garmin-pp-cli which "stale tickets"
  garmin-pp-cli which "bottleneck"
  garmin-pp-cli which --limit 1 "send message"
  garmin-pp-cli which                                # list the full capability index`,
