...
### Join two series the recipes do not join

```bash
garmin-pp-cli sql "SELECT a.id AS activity_id, json_extract(a.data,'$.activityType.typeKey') AS type, round(json_extract(a.data,'$.distance')/1000.0,2) AS km, json_extract(z.data,'$.zoneCount') AS zones FROM resources a JOIN resources z ON z.resource_type='activity_hr_zones' AND z.id=a.id WHERE a.resource_type='activities' ORDER BY a.id DESC LIMIT 10" --agent
```

Every archived series lives in one table, `resources`, keyed by (resource_type, id), so joining two of them is a self-join on the id they share — here the activity feed and the per-activity heart-rate zones. `sql` accepts a single SELECT or WITH statement and runs it against a handle opened read-only, so no query can change the archive whatever it says. Run `garmin-pp-cli sql "SELECT resource_type, count(*) FROM resources GROUP BY 1"` first to see which series this archive holds. `analytics --type activities --group-by activityName` stays the shorthand when one top-level field of one series is all you need, and `search <text> --type activities --data-source local` is the full-text equivalent over the same rows.
